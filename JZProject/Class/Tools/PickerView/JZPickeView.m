...
@interface JZPickeView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
- (void)dateEnsureAction {
    if (self.delegate && [self.delegate respondsToSelector:@selector(addressPickerWithProvince:city:area:)]) {
        [self.delegate addressPickerWithProvince:self.province city:self.city area:self.area];
    }
}
